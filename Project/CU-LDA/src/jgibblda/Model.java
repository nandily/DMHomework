...
public class Model {	
	...
	public boolean initNewModel(LDACmdOption option){
        ...
        String mn[][]=readtxt("./test/ldadata1.txt");//UTF-8 NO BOM
       // System.out.print(mn.length);
		
		String map[][]=readtxt("./test/wordmap.txt");
		//System.out.print(map.length);
		//int m=mn.length;
		String ty[]=readyiwei("./test/tingyong.txt");
		System.out.print("分层模型标记");
		for(m=0;m<mn.length;m++){
			System.out.print("m="+m);
			System.out.print("\r\n");
			for(n=0;n<mn[m].length;n++){
				
				int i,j;
				for(i=0;i<map.length;i++){
					//System.out.print("i="+i);
					//System.out.print("\r\n");
					if(mn[m][n].equals(map[i][0]))
						break;
				}
				for(j=0;j<ty.length;j++){
					//System.out.print("i="+i);
					//System.out.print("\r\n");
					if(mn[m][n].equals(ty[j].trim()))
						break;
				}
				if(j>=ty.length){
				    mn[m][n]=map[i][1];
		...
		try{
			BufferedWriter wr=new BufferedWriter(new FileWriter(new File("./test/tongjidata.txt")));
		...
		//String mn1[][]=readtxt("./test/tongjidata.txt");
		//System.out.print(mn1.length);
		for (m = 0; m < data.M; m++){
			...
			for (n = 0; n < N; n++){
				...
				int lable=Integer.parseInt(mn[m][n]);	
				
				//System.out.print("m="+m);
				//System.out.print("n="+n);
				//System.out.print("lable="+lable);
				int topic=caltopic(lable,K);
		...
		String User[][]=readtxt("./test/ldadata2.txt");
		for (m = 0; m < U; m++){
			int N =User[m].length; 
			z1[m] = new Vector<Integer>();
	...
	static int caltopic(int label,int K){
		int l1=0,l2=1;
		if(label<=l1)
		   {
		...
		if(label==0)
		{
			topic=(int)(Math.random()*(K-4));/////topic 从1开始计数
		}
		else if(label==1)
		{
			topic=(int)(Math.random()*(K-2));
			//topic+=K;
		}
		else
		{
